...
register_name = "glancing_ce"

default_dict = {
    "glancing_label_smoothing": {"type": float, "default": 0.1, "help": "label smoothing"}}
...
class LabelSmoothedCrossEntropyCriterion(_Loss):
    ...
    def __init__(self, config: namedtuple):
        ...
        self.config = config
        self.eps = torch.tensor(self.config.glancing_label_smoothing, dtype=torch.float32)
    ...
    def forward(self,
        ...
        if padding_idx:
            padding_mask = (target_flat == padding_idx)
            mask = (mask.view(-1) | padding_mask)
